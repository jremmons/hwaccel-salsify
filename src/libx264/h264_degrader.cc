...
void H264_degrader::bgra2yuv420p(uint8_t* input, AVFrame* outputFrame, size_t width, size_t height){
  uint8_t * inData[1] = { input };
  int inLinesize[1] = { 4*width };
  
  sws_scale(bgra2yuv420p_context, inData, inLinesize, 0, height, outputFrame->data, outputFrame->linesize);
}

void H264_degrader::yuv420p2bgra(AVFrame* inputFrame, uint8_t* output, size_t width, size_t height){
  ...
  sws_scale(yuv420p2bgra_context, inData, inLinesize, 0, height, outputArray, outLinesize);
}
    ...
    quantization(quantization)
{
  ...
  bgra2yuv420p_context = sws_getContext(width, height,
                                        AV_PIX_FMT_BGRA, width, height,
                                        pix_fmt, 0, 0, 0, 0);
  if (bgra2yuv420p_context == NULL) {
    std::cout << "BGRA to YUV420P context not found\n";
    throw;
  }


  yuv420p2bgra_context = sws_getContext(width, height,
                                        pix_fmt, width, height, 
                                        AV_PIX_FMT_BGRA, 0, 0, 0, 0);

  if (yuv420p2bgra_context == NULL) {
    std::cout << "BGRA to YUV420P context not found\n";
    throw;
  }
...
H264_degrader::~H264_degrader(){
    ...
    sws_freeContext(bgra2yuv420p_context);
    sws_freeContext(yuv420p2bgra_context);
}
