...
int main(int argc, char **argv)
{
    ...
    std::shared_ptr<uint8_t> input_buffer(new uint8_t[(1280*720*3)/2]);
    std::shared_ptr<uint8_t> output_buffer(new uint8_t[(1280*720*3)/2]);
    std::shared_ptr<uint8_t> c(new uint8_t[(1280*720*3)/2]);
    ...
    while(!infile.eof()){
        infile.read((char*)input_buffer.get(), (1280*720*3)/2);
        ...
        size_t len = degrader.encode(input_buffer.get(), c.get());
        degrader.decode(c.get(), len, output_buffer.get());
        ...
        outfile.write((char*)output_buffer.get(), (1280*720*3)/2);
    }
